...
class MPC(Node):
	...
	def __init__(self):
		...
		self.declare_parameter("map", 'map3')
		...
		self.declare_parameter("TK",3)
		self.TK = self.get_parameter("TK").value
		'''Time step horizon'''

		self.Rk = np.diag([0.01, 10.0]) # input cost matrix, penalty for inputs - [accel, steering_speed]
		self.Rdk = np.diag([0.01, 100.0])  # input difference cost matrix, penalty for change of inputs - [accel, steering_speed]
		self.Qk = np.diag([900, 900, 1000, 100, 100])  # state error cost matrix, for the the next (T) prediction time steps 
		self.Qfk = np.diag([13.5, 13.5, 1000, 100, 100 ])  # final state error matrix, penalty  for the final state constraints
	...
	def odom_callback(self, msg: Odometry):
		...
		self.ref_traj = self.calc_ref_trajectory_kinematic(self.state, self.waypoints[:,0], self.waypoints[:,1], self.waypoints[:,4], self.waypoints[:,7])
		...
		self.oa,self.odelta,ox,oy,oyaw,ov,path_predict = self.linear_mpc_control_kinematic(self.ref_traj, x0, self.oa, self.odelta)
		# print(path_predict)
		...
		utils.pubishActuation(self.odelta[0], self.velocity, self.cmd_pub, self.MAX_SPEED)
	...
	def predict_motion_kinematic(self, x0, oa, od, xref):
		...
		for t in range(1, self.TK + 1):
			state = self.update_state_kinematic(state, oa[t-1], od[t-1])
			path_predict[0, t] = state.x
			path_predict[1, t] = state.y
			path_predict[2, t] = state.v
			path_predict[3, t] = state.yaw
	...
	def update_state_kinematic(self, state: State, a, delta):
		...
		state.x = state.x + state.v * math.cos(state.yaw) * self.DTK
		state.y = state.y + state.v * math.sin(state.yaw) * self.DTK
		state.yaw = (state.yaw + (state.v / self.WB) * math.tan(delta) * self.DTK)
		state.v = state.v + a * self.DTK
	...
	def calc_ref_trajectory_kinematic(self, state: State, cx, cy, cyaw, sp):
		...
		# sp[sp>=self.MAX_SPEED] = self.MAX_SPEED
	...
	def mpc_prob_init(self):
		...
		for t in range(self.TK):  
			A, B, C = self.get_model_matrix(path_predict[2, t], path_predict[3, t], 0.0)  # reference steering angle is zero
			A_block.append(A)
			B_block.append(B)
			C_block.extend(C)
		...
		constraints += [cvxpy.abs(cvxpy.diff(self.uk[1, :])) <= self.MAX_DSTEER * self.DTK]
	...
	def get_model_matrix(self, v, phi, delta):
		...
		A[2, 2] = 1.0
		A[3, 3] = 1.0
		A[0, 2] = self.DTK * math.cos(phi)
		A[0, 3] = -self.DTK * v * math.sin(phi)
		A[1, 2] = self.DTK * math.sin(phi)
		A[1, 3] = self.DTK * v * math.cos(phi)
		A[3, 2] = self.DTK * math.tan(delta) / self.WB

		# Input Matrix B; 4x2
		B = np.zeros((self.NXK, self.NU))
		B[2, 0] = self.DTK
		B[3, 1] = self.DTK * v / (self.WB * math.cos(delta) ** 2)

		C = np.zeros(self.NXK)
		C[0] = self.DTK * v * math.sin(phi) * phi
		C[1] = -self.DTK * v * math.cos(phi) * phi
		C[3] = -self.DTK * v * delta / (self.WB * math.cos(delta) ** 2)
