...
class MPC(Node):
	...
	def __init__(self):
		...
		map_path = f'src/global_planning/maps'
		self.waypoints = np.loadtxt(map_path + '/' + self.map_name + '_short_minCurve.csv', delimiter=',', skiprows=1)  # csv data
	...
	def pose_callback(self, pose_msg):
		...
		ref_path = self.calc_ref_trajectory(vehicle_state, self.waypoints[:, 0], self.waypoints[:, 1], self.waypoints[:, 4], self.waypoints[:, 7])
		# print(ref_path)
	...
	def update_vehicle_state(self, pose_msg):
		...
		vehicle_state.yaw = self.euler_from_quaternion(q[0],q[1],q[2],q[3])
