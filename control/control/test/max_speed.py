...
class myNode(Node):
	def __init__(self):
		...
		# self.Rk = np.diag([0.01, 1])  # input cost matrix
		...
		# self.Rdk = np.diag([0.01, 1/0.9])  # input difference cost matrix
		# self.Qk = np.diag([1/0.01, 1/0.01, 1/0.01, 3, 0])  # state cost matrix
		# self.Qfk = np.diag([1/0.01, 1/0.01, 1/0.01, 3 ,0])  # state final cost matrix
	...
	def odom_callback(self, msg: Odometry):
		...
		self.ref_traj, self.wall_l, self.wall_r = self.calc_ref_trajectory(self.state, self.waypoints[:,0], self.waypoints[:,1], self.waypoints[:,4], self.waypoints[:,7])
	...
	def mpc_prob_solve_kinematic(self, ref_traj, path_predict, x0):
		self.x0k.value = x0
		self.wr.value = self.wall_r
		self.wl.value = self.wall_l
	...
	def init_mpc(self):
		...
		self.wr = cvxpy.Parameter((self.step_horizon,))
		self.wr.value = np.zeros((self.step_horizon,))

		self.wl = cvxpy.Parameter((self.step_horizon,))
		self.wl.value = np.zeros((self.step_horizon,))
	...
	def calc_ref_trajectory(self, state: State, cx, cy, cyaw, sp):
		...
		wall_r = np.zeros(self.step_horizon + 1)
		wall_l = np.zeros(self.step_horizon + 1)
		ncourse = len(cx)
		...
		wall_r[0] = self.waypoints[ind, 2]
		wall_l[0] = self.waypoints[ind, 3] 
		...
		wall_r[:] = self.waypoints[ind_list, 2]
		wall_l[:] = self.waypoints[ind_list, 3]

		return ref_traj, wall_l[:-1]*-1, wall_r[:-1]
