...
class MPC(Node):
	...
	def __init__(self):
		...
		self.Qk = np.diag([50, 50, 100, 13.0])  # state error cost matrix, for the the next (T) prediction time steps 
		self.Qfk = np.diag([13.5, 13.5, 5.5, 13.0])  # final state error matrix, penalty  for the final state constraints
	...
	def odom_callback(self, msg: Odometry):
		...
		utils.pubishActuation(self.odelta[0], self.velocity, self.cmd_pub)
